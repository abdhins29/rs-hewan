...
<?php 
	$no = 1;
	$q = mysqli_query($konek,"SELECT * FROM tbl_reservasi a LEFT JOIN tbl_regishewan b ON a.kd_pasien=b.kd_pasien LEFT JOIN tbl_pelayanan c ON a.kd_pelayanan=c.kd_pelayanan WHERE kd_reservasi = '$kd_reservasi'");
...
<div class="container-fluid">
	<table class="table table-responsive" border="1" width="80%" style="margin-left: 7%;">
		...
		<?php 
			$qq = mysqli_query($konek,"SELECT * FROM tbl_reservasi a LEFT JOIN tbl_pelayanan b ON a.kd_pelayanan=b.kd_pelayanan WHERE kd_reservasi='$kd_reservasi'");
